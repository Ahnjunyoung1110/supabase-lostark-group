...
- [ ] 앱 어디에도 사용자 UUID/raw Discord id가 일반 표시명으로 노출되지 않는다.
- [ ] 사용자는 `/profile`에서 닉네임을 설정/수정할 수 있고, 주요 화면에 반영된다.
- [ ] `/events`에서 주최자가 본인 약속을 여러 개 선택해 삭제할 수 있다.
- [ ] 약속 상세 또는 생성 직후 화면에서 Discord 공유 문구를 복사할 수 있다.
- [ ] webhook 설정 시 Discord 채널로 약속 공유 메시지를 전송할 수 있다.
