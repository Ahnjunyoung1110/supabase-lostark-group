...
| v3-2.3 스펙 이력 추적 + 그래프 (`/characters/[id]`) | ✅ 완료 | — |
| v3-3 Discord 버튼 응답 (Bot + Interaction) | 🚧 코드·DB 완료 / 인프라 설정 대기 | `acc2837` |
...
supabase/migrations/20260619190000_add_discord_user_id.sql  # Discord user id 매핑 (원격 적용 완료)
lib/discord/user-id.ts                                  # Supabase Discord OAuth id 추출 헬퍼
lib/discord/verify-signature.ts                         # Discord Interaction Ed25519 서명 검증
app/api/discord/interactions/route.ts                   # Discord 버튼 Interaction endpoint
supabase/migrations/20260623090000_add_character_snapshots.sql  # 스펙 이력 테이블 (원격 적용 완료)
supabase/functions/update-character-specs/index.ts      # 스냅샷 INSERT 로직 추가 (변화 시에만)
lib/characters.ts                                       # CharacterSnapshotRow 타입 추가
lib/queries.ts                                          # getCharacterById(), getCharacterSnapshots() 추가
components/spec-history-chart.tsx                       # SVG 라인 차트 (서버 컴포넌트, 의존성 0)
app/characters/[id]/page.tsx                            # 캐릭터 상세 + 스펙 이력 그래프
components/character-card.tsx                           # 이력 보기(History) 아이콘 버튼 추가
```

### 주의사항 (pg_cron)
원격 DB role은 `alter database postgres set app.*` 실행 권한이 없어 `current_setting('app.edge_function_url')` 방식이 실제 호출 없이 `0 rows`로 끝날 수 있다.  
...
코드·DB 완료 (`acc2837`, `20260619190000` 원격 적용 완료). 남은 작업은 **모두 인프라 수동 설정**:

1. ~~`npx supabase db push --linked` — `20260619190000_add_discord_user_id.sql` 원격 적용~~ ✅ 완료
2. Discord Developer Portal에서 Application 생성 + Interaction Endpoint URL 등록/Verify
3. 배포 환경(Vercel) 환경변수 설정: `DISCORD_PUBLIC_KEY`, `DISCORD_BOT_TOKEN`, `DISCORD_CHANNEL_ID`, `DISCORD_APPLICATION_ID`, `SUPABASE_SERVICE_ROLE_KEY`, `NEXT_PUBLIC_SITE_URL`
4. Bot 서버 초대
5. 실제 버튼 클릭 → `event_responses` 반영 확인

> 코드 연결 상태: `buildDiscordBotMessagePayload`(버튼 3종) → `shareEventToDiscord` → share-panel "Discord 채널로 전송" 완전 연결됨. 로컬 `.env.local`엔 Discord 변수 없음 — Vercel 대시보드에서 설정 필요.
...
- 젬 효율은 lopec `specPoint`의 Next.js Flight payload와 client chunk 계산기를 사용해 화면의 최종 `젬` 효율과 동일한 값을 저장한다. 원시 활성 젬 옵션 합계는 fallback으로만 사용한다.
- 팔찌·각인·메인노드 효율은 lopec `efficiency` 페이지 카드 값에서 수집
- 원격 DB 마이그레이션 및 `update-character-specs` Edge Function 배포 완료
- 2026-06-22 점검: `낼름낼름개구리` 젬 효율이 원시 옵션 합계 `8.95`로 저장되던 문제를 수정했고, 수동 invoke 후 DB 값 `10.85`, breakdown `{ efficiency: 10.85, optionEfficiency: 8.44, pointEfficiency: 2.22 }` 확인 완료.
...
### v3-2.3 스펙 이력 추적 + 캐릭터 상세 페이지

**목표:** 매 갱신마다 스펙 변화를 누적 저장하고, `/characters/[id]`에서 아이템레벨·환산점수 변화를 SVG 라인 차트로 보여준다.

**변경 파일 목록**

| 파일 | 역할 |
|---|---|
| `supabase/migrations/20260623090000_add_character_snapshots.sql` | `character_snapshots` 테이블 + 인덱스 + RLS (원격 적용 완료) |
| `supabase/functions/update-character-specs/index.ts` | 성공 갱신 직후 직전 스냅샷과 비교 → 변화 시에만 INSERT (Edge Function 재배포 완료) |
| `lib/characters.ts` | `CharacterSnapshotRow` 타입 추가 |
| `lib/queries.ts` | `getCharacterById()`, `getCharacterSnapshots()` 추가 |
| `components/spec-history-chart.tsx` | 순수 SVG 라인 차트 — 서버 컴포넌트, hook 없음, 의존성 0 |
| `app/characters/[id]/page.tsx` | 캐릭터 상세 페이지 — 헤더 + 효율 지표 + 이력 그래프 2개 |
| `components/character-card.tsx` | `History` 아이콘 버튼 추가 → `/characters/{id}` 링크 |

**아키텍처 결정**

- **스냅샷 기록 위치**: Edge Function `index.ts`의 성공 UPDATE 블록 직후. 실패 경로(fetch_error만 기록하는 두 블록)에서는 스펙 값이 없으므로 INSERT 안 함.
- **변화 판정 기준**: `item_level` 또는 `spec_score` 중 하나라도 직전 스냅샷과 다르면 INSERT. 첫 갱신은 직전 스냅샷이 없으므로 항상 INSERT.
- **RLS**: SELECT는 인증 사용자 전체 열기(`using (true)`). INSERT/UPDATE/DELETE 정책 없음 — Edge Function의 service role이 RLS 우회하여 기록. 일반 클라이언트는 쓰기 불가.
- **그래프 렌더링**: 외부 차트 라이브러리 미설치 상태라 SVG `<polyline>` 직접 구현. 서버 컴포넌트로 유지(hook 없음). 데이터 포인트가 1개 이하면 "이력 데이터 부족" 안내 메시지 표시.
- **상세 페이지 접근 권한**: 모든 인증 사용자가 타인 캐릭터 상세도 조회 가능(그룹 비교 맥락).

**주의점 / 오래 걸릴 수 있는 함정**

- Edge Function은 `npx supabase functions deploy update-character-specs`로 별도 배포해야 한다. `git push`나 Vercel 배포만으로는 반영 안 됨.
- `character_snapshots` 테이블 생성 후 기존 캐릭터에 과거 이력은 없다. 첫 갱신 시점부터 스냅샷이 쌓이기 시작한다. 그래프 선이 그려지려면 **값 변화가 있는 갱신 2회 이상** 필요.
- `maybeSingle()` 사용: Supabase `limit(1).single()`은 결과가 없으면 에러를 던지므로 `maybeSingle()`을 사용해야 `lastSnap === null` 처리가 가능하다.
- DB에서 내려오는 numeric 컬럼은 JS에서 `string` 또는 `number`로 올 수 있어, 비교 시 `Number(lastSnap.item_level)` 변환 처리를 적용했다.
- SVG `fill`/`stroke` 색상은 Tailwind 클래스 대신 `hsl(var(--primary))` 인라인 스타일로 지정. SVG 속성과 Tailwind 유틸리티 클래스 간 호환 문제를 피하기 위함.
...
npx supabase db query --linked -o table \
  "select character_id, item_level, spec_score, fetched_at from public.character_snapshots order by fetched_at desc limit 10;"
```

수동 시나리오:
1. 카드 `History` 아이콘 클릭 → `/characters/{id}` 진입 확인.
2. 캐릭터 새로고침 → `character_snapshots` 행 INSERT 확인 (첫 갱신).
3. 값 변동 없이 재갱신 → 행 수 그대로 (변화 시에만 기록 검증).
4. 값이 다른 두 스냅샷 존재 시 `/characters/{id}` 그래프 선 표시 확인.
5. 스냅샷 1개 상태에서는 "이력 데이터 부족" 메시지 표시 확인.
...
- [x] `/characters` 페이지에서 계정당 최대 3개 캐릭터를 등록/삭제할 수 있다.
- [x] 등록 시 lopec.kr에서 환산점수·아이템레벨·직업·티어를 자동으로 가져온다.
- [x] lopec 실패 시 공식 로아 API로 폴백해 최소 정보를 표시한다.
- [x] 수동 새로고침과 1시간 자동 갱신이 동작한다.
- [x] 그룹 전체 캐릭터를 환산점수 순으로 비교하는 테이블이 있다.
- [x] 스펙 이력이 누적 저장되고 `/characters/[id]`에서 변화 그래프를 볼 수 있다.
- [ ] Discord 메시지 버튼으로 참석/불참/미정을 응답하면 앱에 즉시 반영된다. ← **코드 완료, 인프라 설정 대기**
- [ ] Discord 서명 검증 없는 요청은 거부된다. ← 코드 완료 (인프라 설정 후 자동 동작)
- [ ] 앱에 매핑되지 않은 Discord 사용자는 안내 메시지를 받는다. ← 코드 완료 (인프라 설정 후 자동 동작)
